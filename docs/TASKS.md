...
| Item | Status | Notes |
|---|---|---|
| Shared task-component interface | ✅ | `Task = CodeTask \| ParsonsTask \| QuizTask \| PredictTask \| FixTask` (`lib/task/types.ts`); `components/task/TaskWorkspace.tsx` dispatches by `task.type` to `components/task-types/*View.tsx`. `lib/db/tasks.ts` and the session/practice routes hand back the union — the next type (`fill`) is a new branch plus a new file, not a rewrite |
...
| First topic, ~12 tasks | 🔶 | Five tasks across five topics: `content/seed-tasks/grade7-turtle-square.json` (turtle-basics), `grade7-parsons-triangle.json` (turtle-loops), `grade7-quiz-variable-names.json` (variables), `grade7-predict-arithmetic.json` (arithmetic), `grade7-fix-square.json` (debugging), imported by `npm run db:seed` |
...
9. ~~Task authoring UI~~ — done, scoped to `code` tasks: `/tasks`, `/tasks/new`, `/tasks/[id]`
   ...
   are still open.
10. ~~Remaining task types, `parsons` first~~ — `parsons` (`indentMode: 'given'`), `quiz`,
    `predict` (`answerMode: 'text'`) and `fix` are done end to end (docs/TASK_SCHEMA.md); parsons
    `'chosen'`, predict `'choice'`/`imageOptions`, and `fill` remain. The shared task-component
    interface this needed (`Task` union, `TaskWorkspace` dispatching by type) now exists too, so
    the next type is a new branch and a new file, not a rewrite.
