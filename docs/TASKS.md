...
| Playback scrubber with line highlighting | ✅ | `components/canvas/PlaybackScrubber.tsx` + `lib/canvas/playback.ts`. `Segment.line` is always null, so stepping is driven by position in the segment array (call order), highlighting the last-drawn segment on `TurtleCanvas` rather than a source line. Wired into `code`, `fix`, `fill`. Covers grade 7 lesson 40 without an interpreter stepper. Shown only where a task draws (`lib/task/surface.ts`'s `showsTurtleCanvas`): `surface: 'turtle'` for `code`/`fix`, a shape check or a turtle import for `fill` (which has no surface field), or any run that actually produced segments — a console task no longer sits next to an empty canvas. Unit-tested; `tests/e2e/next-task.spec.ts` covers console code, console fix, turtle code and turtle fill |
...
| Next-task button after a passed Check | ✅ | `components/task/NextTaskButton.tsx`, handed to `TaskWorkspace` as `next` and rendered inside the success result of all six task types — the views never know what "next" means. Practice: the lesson's next step (`lib/lessons/view.ts`'s `stepAfter`), after the last one «Повернутися до уроку». Session: the next task in the teacher's order that is neither passed nor locked (`lib/session/next-task.ts`'s `nextOpenTaskId`, unit-tested), after the last one «Повернутися до списку завдань». In a graded session the first Check swaps the workspace for the locked screen, so the button lives there, on a passed lock only; moving on never touches the lock, and a locked task is skipped rather than reopened. `tests/e2e/next-task.spec.ts` |
