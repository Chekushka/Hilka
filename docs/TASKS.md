...
| `code` | 🔶 | Turtle surface end to end. Console surface: plain Run now works via the interactive input line (above) — a student can write and try their own `input()`-calling program. Check does not: `tasks.cases`/`RunCase` already exists end to end in the schema (`lib/db/schema.ts`) and the checker (`lib/checker/reference-check.ts`'s `checkTaskReference` already runs a reference once per case), but `cases` is dropped at the DB→Task mapping (`lib/db/task-mapping.ts` never copies `row.cases`), missing from `CodeTask`/`FixTask` (`lib/task/types.ts`), never read by `useTaskRunner`'s Check path (always headless with no stdin), has no authoring surface (`NewTaskForm`/`DraftTaskEditor`, the publish route), and — the blocker underneath all of that — `useTaskRunner`'s warm-up runs `reference.code` headless with no stdin to seed `target`, which throws for any reference that calls `input()` before a case can even supply one. A console task with `input()` cannot be authored or graded yet; TASK_SCHEMA.md's BMI worked example does not run |
