...
| Additional tasks for fast students | 🔶 | The mechanism exists — each lesson's `additional_task_ids`, shown after the core tasks ("Lessons", above). What is missing is content: most lessons have none yet |
...
| Grade 7 sem-2 block: intro → loops-for + turtle-basics | 🔶 | All 9 topics in this range now have 3 tasks each instead of 1 (intro 3, variables 3, arithmetic 3, linear 3, turtle-basics 3, conditions 3, turtle-conditions 4, loops-while 3, loops-for 3) — real practice, but still short of the ~12/topic CURRICULUM.md sizes a full lesson at. |
...
| Mandatory vs practice lessons | ✅ | A `lessons` table (see "Lessons", above), not a tag on topics/tasks. Grade 7 lessons 25–38 and 40 seeded from `content/lessons/grade7.json` + one explanation `.md` per lesson, mapped from the existing seed tasks — 15 lessons, 10 mandatory, 5 practice. **Content gaps**: lessons 39 (conditions-nested) and 41–42 (turtle-animation) have no tasks yet and are left out rather than shown empty; most practice lessons have one core task and no additional ones — the next content pass fills them |
