...
/**
 * A draft `quiz` task: edit its fields, then publish. Like `parsons`,
 * nothing executes and there is no reference solution to write — the
 * correct answer lives entirely in `checks` (`choice_equals.indices`), so
 * Publish alone posts the saved fields straight to lib/task/quiz.ts's
 * consistency check.
 */
...
interface QuizDraftEditorProps {
  task: DraftQuizTask;
}
...
export function QuizDraftEditor({ task }: QuizDraftEditorProps) {
  ...
  const [multiple, setMultiple] = useState(task.payload.multiple);
  ...
  async function save(): Promise<{ ok: true } | { ok: false }> {
    ...
    if (parsedOptions.length === 0) {
      setSaveState('error');
      setSaveError(t('authoring.quizOptionsEmpty'));
      return { ok: false };
    }
    setSaveState('saving');
    setSaveError(null);
    const response = await fetch(`/api/tasks/${task.id}`, {
      ...
      body: JSON.stringify({
        title,
        payload: { type: 'quiz', prompt, options: parsedOptions, multiple },
      ...
      <section className="flex flex-col gap-3 border-t border-line pt-6">
        ...
        {publishState.kind === 'checks_failed' && (
          <div className="text-sm text-attention">
            <p>{t('authoring.quizChecksFailed')}</p>
