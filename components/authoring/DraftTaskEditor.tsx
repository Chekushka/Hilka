...
/**
 * A draft `code` task: edit its fields, then run and publish its reference
 * solution. One component, not two, because publish must evaluate the
 * checks and payload as they are on the server — so Publish always saves
 * first, in sequence, rather than trusting whatever the form last saved.
 *
 * The reference solution runs in this browser, not on a server (there is no
 * server-side Python execution — docs/AI_CONTEXT.md). Publish posts the
 * resulting run to POST /api/tasks/[id]/publish, which re-evaluates it
 * against the checks it just saved before writing anything.
 */
