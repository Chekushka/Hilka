...
import { toCodeTask, toParsonsTask, toTask, type TaskRow } from './task-mapping';
...
describe('toTask', () => {
  ...
  it('returns null for a type nothing maps yet', () => {
    expect(toTask(row({ type: 'quiz', payload: { type: 'quiz' } as unknown as TaskRow['payload'] }))).toBeNull();
  });
});
